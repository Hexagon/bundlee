 ...
 */
async function recursiveReaddir(path: string, extensionFilter?: string[]): Promise<string[]> {
  const files: string[] = []
 ...
 */
async function bundleFiles(basePath: string, fileList: string[]): Promise<Record<string, string>> {
  ...
  for (const file of fileList) {
    ...
    result[relativePath] = encode((await (dst.readable.getReader()).read()).value || "")
  }
  return result
}
...
export async function bundlePath(
  ...
  const fileList = await recursiveReaddir(join(basePath, path), exts)

  if (!fileList || fileList.length === 0) {
    throw new Error("No input files found")
  }

  return await bundleFiles(basePath, fileList)
}
 ...
 */
export async function fileContentFromBundle(
  filePath: string,
  bundlePath: string,
): Promise<string> {
  const bundleFile = await import(bundlePath, {
    assert: { type: "json" },
  })
  const encodedFileContent = bundleFile.default[filePath]
  if (encodedFileContent) {
    ...
    const decodedFileContent = new TextDecoder().decode(await readAll((await (dest.readable.getReader()).read()).value))

    return decodedFileContent
  } else {
    throw new Error("File not found in bundle")
  }
}
